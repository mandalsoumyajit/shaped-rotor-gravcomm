...
if __name__=='__main__':
 ...
 configs=json.loads((OUT/'shortlist.json').read_text());stage='candidate_gates' if a.gates else 'refinement';folder=OUT/stage;folder.mkdir(exist_ok=True)
 if not a.gates:
  configs=[c for c in configs if json.loads((OUT/'candidate_gates'/f'{c["id"]}_10000.json').read_text())['passes_detector_gate']]
 jobs=[(c,i,a.gates) for c in configs for i in ([10000] if a.gates else range(20000,20000+a.packets))]
 manifest=dict(configs=configs,jobs=len(jobs),packet_start=10000 if a.gates else 20000,sha256={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),ROOT/'scripts/streaming_link_search.py',*(ROOT/'src/gravcomm').glob('*.py')]})
