"""Report the controlled tone-spacing/continuous-duty comparison."""
import json,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.path.insert(0,str(ROOT/'src'))
from gravcomm.packet_confidence import packet_ber_interval
OUT=ROOT/'results/sequence_cf_fsk'
r=json.loads((OUT/'torque_relaxed_ber.json').read_text())
interval=packet_ber_interval(r['bit_errors_per_packet'])
assert interval[1]<1e-3,'Receiver-only test has not yet established the BER target'
r['packet_aware_anytime95_ber_interval']=interval
r['interpretation']='BER threshold test; low error counts are reported as a confidence bound'
(OUT/'torque_relaxed_ber.json').write_text(json.dumps(r,indent=2)+'\n')
report=f'''# Rate-limiting constraints
...
| 1.75 s | 0.09524 Hz | 1.524 | 95% upper bound {interval[1]:.3g} | 0.583 N m | 0.285 N m |

The final row has {r['bit_errors']} bit errors in {r['bits']:,} bits from
{r['packets']:,} independent packets. Its packet-aware interval is
[{interval[0]:.3g}, {interval[1]:.3g}]. This row is used to test the BER threshold;
...
'''
(OUT/'BOTTLENECK.md').write_text(report)
print(report)
