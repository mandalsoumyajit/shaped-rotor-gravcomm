...
OUT=ROOT/'results/scaled_replacements_2026_09_20'
FROZEN=ROOT.parent/'results/fixed_rate_sizing_2026_09_20/ber_replacement_v2/frozen_designs.json'
def save(p,x):
...
def prepare():
 ...
 for h in (.018,.013):
  ...
  for i,ch in enumerate(frozen['channels']):
   for label,fc in [('low',18-3*spacing),('center',18),('high',18+3*spacing)]:specs.append(dict(name=f'design{i+1}_{label}',scale=ch['source_designs'][0]['scale'],kind='centrifugal',rpm=30*fc,design=i+1))
  for spec in specs:
 ...
 save(OUT/'manifest.json',manifest);return jobs
def solve(job):
