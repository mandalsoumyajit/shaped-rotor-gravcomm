...
def main():
 ...
 for h in (.018,.013):
  ...
  src=ROOT/f'results/design_sweep/medium_rounded/h{h:g}_rpm1800/model.inp';nodes,elements,materials=read_input(src);eid=np.array(sorted(elements));conn=np.array([elements[int(e)] for e in eid]);lookup={int(n):i for i,n in enumerate(ids)};ix=np.array([[lookup[int(n)] for n in c] for c in conn]);X=xyz[ix];vol=np.linalg.det(np.stack((X[:,1]-X[:,0],X[:,2]-X[:,0],X[:,3]-X[:,0]),axis=2))/6
  ...
  for i,ch in enumerate(manifest['frozen']['channels'],1):
   ...
   omega_max=np.pi*(18+3*.08/(.4*(224/162)));alpha=source['peak_torque_nm']/source['inertia_kg_m2']
   ...
   for sign in (-1,1):
    for t in np.linspace(0,1,21):
     smooth=10*t**3-15*t**4+6*t**5;f=18+sign*3*spacing*(2*smooth-1);a=sign*np.pi*6*spacing*(30*t*t*(1-t)**2)/transition
   ...
   row=dict(design=i,mesh=h,mass_kg=source['mass_kg'],scale=scale,mesh_mass_kg=stats['mesh_mass_kg']*scale**3,mesh_inertia_kg_m2=stats['mesh_inertia_kg_m2']*scale**5,centrifugal_raw_alu_vm_mpa=float(max(vm(ss)[alu])/1e6),centrifugal_raw_steel_vm_mpa=float(max(vm(ss)[~alu])/1e6),centrifugal_max_u_um=float(max(np.linalg.norm(uu,axis=1))*1e6),combined_conservative_alu_vm_mpa=float(max(bound[alu])/1e6),combined_conservative_steel_vm_mpa=float(max(bound[~alu])/1e6),combined_conservative_u_um=float(max(ubound)*1e6),sampled_transition_gravity_bound_vm_mpa=max(v[0] for v in phase_peaks)/1e6,sampled_transition_gravity_bound_u_um=max(v[1] for v in phase_peaks)*1e6,angular_only_max_u_um=float(max(np.linalg.norm(au,axis=1))*1e6),gravity_any_attitude_u_bound_um=float(max(np.sqrt(sum(np.sum(g*g,axis=1) for g in gu))))*1e6,regional_10mm_scaled_vm_mpa=float(max(np.average(bound[mask],weights=weights[mask]) for mask in windows[16:])/1e6))
   baseline=json.loads((src.parent/'metadata.json').read_text())['summary']
   predicted_u=baseline['max_displacement_m']*scale**3*(omega_max/(60*np.pi))**2
   row['centrifugal_displacement_similarity_error']=abs(row['centrifugal_max_u_um']/1e6/predicted_u-1)
   assert row['centrifugal_displacement_similarity_error']<.002
   predicted_vm=baseline['max_aluminum_integration_point_vm_pa']*scale**2*(omega_max/(60*np.pi))**2
   row['centrifugal_alu_stress_similarity_error']=abs(row['centrifugal_raw_alu_vm_mpa']*1e6/predicted_vm-1)
   assert row['centrifugal_alu_stress_similarity_error']<.002
   results.append(row)
 ...
 save(OUT/'static_compiled.json',results);save(OUT/'equilibrium_checks.json',checks);save(OUT/'modal_frequencies.json',modal)
